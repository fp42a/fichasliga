...
2. Descomprime el zip. En **Add file → Upload files** arrastra primero la **carpeta `api`** completa (la carpeta, no los archivos de adentro) y después los archivos sueltos. Luego haz clic en **Commit**.
   En la página principal del repositorio debe verse una carpeta `api` y, junto a ella, `index.html`, `app.js`, `styles.css`, `jspdf.umd.min.js`, `package.json` y `vercel.json`.
...
- **Otra tabla** (por ejemplo, Clausura): define `AIRTABLE_TABLE_ID` en Vercel. La tabla nueva debe tener los mismos IDs de campo. Si no, actualiza `CONFIG.fields` en `api/_core.js`.
...
| `api/_core.js` | Lectura de Airtable, agrupación y firmas (el `_` indica a Vercel que no es un endpoint) |
| `index.html`, `app.js`, `styles.css` | Interfaz y generación del PDF |
| `jspdf.umd.min.js` | Librería jsPDF 2.5.2 (MIT) |
